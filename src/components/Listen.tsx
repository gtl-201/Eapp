...
import Right from 'src/asset/audio/right.wav';
import Wrong from 'src/asset/audio/wrong.ogg';
...
const Liste: React.FC<ListeProps> = ({
    ...
    return (
      <View>
        {numWrong[0] == item.id ||
          ...
          numWrong[8] == item.id ||
          numWrong[9] == item.id ?
          <TouchableOpacity onPress={() => TTSP(item.eng)} style={[tailwind('mx-5 bg-red-200 my-2 px-3 py-2 rounded-xl bg-white justify-center items-center'), styles.shadow2]}>
      ...
      <View style={tailwind('pt-7 h-full')}>
        {
          randomIndex == 0 ?
            <View style={tailwind('px-3 justify-center')}>
              {speak == true && TTSP(item.eng)}
              {speak == true && setSpeak(false)}
              <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#e95950', '#bc2a8d']} style={[tailwind('bg-black rounded-xl m-2 w-5/12 self-center')]}>
                <TouchableOpacity onPress={() => TTSP(item.eng)} style={[tailwind('py-2 items-center self-center justify-center rounded-xl w-5/12')]}>
                  <Icon name={"pulse"} size={30} color={'black'} style={tailwind('')}></Icon>
                </TouchableOpacity>
              </LinearGradient>
              ...
              <TouchableOpacity style={[tailwind('px-5 my-1 bg-white rounded-lg border-2 border-gray-300'), { paddingVertical: 7 }]} onPress={() => { play('flase'), setVi(item.vi), setNa(item.na), setId(item.id), setEng(item.eng), setNextQest(true), setIsRigh('false') }}><Text style={[tailwind('text-left text-xl text-black font-bold capitalize')]}>{item.wrongVi[WrongChoose[3]].content}</Text></TouchableOpacity>
              ...
              <View style={tailwind('px-3 justify-center')}>
                ...
                <View style={tailwind('px-3 justify-center')}>
                  ...
                  <View style={tailwind('px-3 justify-center')}>
                    {speak == true && TTSP(item.eng)}
                    {speak == true && setSpeak(false)}
                    <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#e95950', '#bc2a8d']} style={[tailwind('rounded-xl m-2 w-5/12 self-center')]}>
                      <TouchableOpacity onPress={() => TTSP(item.eng)} style={[tailwind('py-2 items-center self-center justify-center r-blue-400 w-5/12')]}>
                        <Icon name={"pulse"} size={30} color={'black'} style={tailwind('')}></Icon>
                      </TouchableOpacity>
                    </LinearGradient>
                    <Text style={tailwind('text-center text-xl text-red-600 capitalize mt-1')}>--= tab to your answer =--</Text>
